...
| D7 | `ρ ∈ {regex, BM25, embedding, LLM}` (paper); `VCC.py --grep` regex-only | `safeRegex` + BM25-lite (K=1.2, B=0.75, floor 0.2, cap, 3 s budget) — already covers the paper's regex **and** BM25 | No change; embedding/LLM ρ need model calls — deferred, revisit only on Phase 7 evidence |
