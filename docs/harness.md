...
| 2.4 | **Commands** `/omp-vcc [keep:N] [focus]` (primary, compact only), `/pi-vcc` (alias), `/vcc-recall` + `/pi-vcc-recall`, `/vcc-stats` (single) | User-facing manual compaction (`keep:N` + free-text focus) and recall without code. `/omp-vcc` compacts with single-line toast only; detailed savings via `/vcc-stats`. No `commands/*.md` file shims — extension-only to avoid duplicate `/omp-vcc`. | → `extensions/main.ts:164-210` `omp-vcc` (compact only, toast) + `213-258` `pi-vcc` alias, `259-303` `vcc-recall` + `305-342` `pi-vcc-recall`, `344` `registerVccStatsCommandHook`, `hook.ts:1265-1292`, `package.json:32` `files` no `commands` |
...
| 2.4 | 5 command surfaces (`omp-vcc`, `pi-vcc` alias, `vcc-recall`, `pi-vcc-recall`, `vcc-stats` single — `/omp-vcc` compact only, toast) | `read extensions/main.ts:164-337` + `read package.json:32-36` | 5 `pi.registerCommand` sites, `/omp-vcc` compacts + toast only; detail via `/vcc-stats` `formatLastStatsDetail` |
