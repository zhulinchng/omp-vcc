...
| 2.4 | **Commands** `/omp-vcc [keep:N] [focus]` (primary), `/pi-vcc` (alias), `/vcc-recall` + `/pi-vcc-recall`, `/vcc-stats` + `/omp-vcc-stats` + inline `/omp-vcc --stats` | User-facing manual compaction (`keep:N` + free-text focus) and recall without code; `--stats` shows savings *without* compacting. Command shims `commands/*.md` are fallback discovery if host scans markdown. | → `extensions/main.ts:164-223` `omp-vcc` + `224-257` `pi-vcc` alias, `260-303` `vcc-recall` + `305-342` `pi-vcc-recall`, `344` `registerVccStatsCommandHook`, `hook.ts:1265-1292`, `package.json:37-40` `commands`, `commands/omp-vcc.md`, `commands/vcc-recall.md` |
...
| 2.6 | **Settings file** `~/.omp/omp-vcc/config.json` via `scaffoldSettings()` | Persisted toggles without touching host `settings-schema.ts` global schema. XDG-aware resolution; migrates legacy `~/.pi/agent/pi-vcc-config.json` once (copies values, never clobbers existing file). | → `extensions/vcc-core/core/settings.ts:9-24` path fn + `26-67` `DEFAULT_SETTINGS` + `110-151` `scaffoldSettings()` |
| 2.7 | **Manifest settings** 5 booleans `vccEnabled, overrideDefaultCompaction, smartKeepTail, continueAfterThresholdCompact, debug` | Appear in `/settings` under plugin section ` @zhulinchng/omp-vcc` and via `omp config set plugins."@zhulinchng/omp-vcc".*` — plugin-scoped per `packages/coding-agent/src/discovery/loader.ts:125` `omp > pi` precedence, not global `compaction.*`. Dual `omp`+`pi` blocks for `pi` backward compat. | → `package.json:41-67` `omp.settings` + `69-104` `pi.settings` (identical), `extensions/vcc-core/core/settings.ts:26-67` `PiVccSettings` interface |
...
sequenceDiagram
  ...
  Factory->>Host: pi.registerCommand omp-vcc<br/>/pi-vcc alias, --stats inline
...
**Counts** at this checkout: 3 event handlers (`context`, `before_agent_start`, dual `session_*`), 2 tools (`vcc_recall`, `vcc_stats`), 5 command registrations (`omp-vcc`, `pi-vcc`, `vcc-recall`, `pi-vcc-recall`, `vcc-stats`+`omp-vcc-stats` alias) plus inline `/omp-vcc --stats` — `extensions/main.ts:164-344` and `hook.ts:708-1292`.
...
- **`vcc_stats`/`vcc-stats`/`/omp-vcc --stats`** `hook.ts:1233-1292` + `extensions/main.ts:174-190` inline — `history:true` → full table; `No compactions yet.` guard when both `last` and `history` empty.
...
| 2.4 | 3+ command surfaces (`omp-vcc`, `pi-vcc` alias, `vcc-recall`, `vcc-stats`/`omp-vcc-stats`, `--stats` inline) | `read extensions/main.ts:164-344` + `read package.json:37-40` + `read commands/omp-vcc.md` | 5 `pi.registerCommand` sites + inline `--stats` branch at `main.ts:174` |
