...
> **omp-vcc interplay**: Snapcompact and VCC both archive `messagesToSummarize` — they are alternative history paths. With default `override:true`, snapcompact is dormant for auto thresholds and explicit `/compact snapcompact` is served by VCC as a generic compaction (the mode never reaches the `session_before_compact` event); the `compactMode` event-field bypass is patch-contract only. Sequential VCC → snapcompact needs `override:false` on the second call (`/omp-vcc` then `/compact snapcompact`). VCC+shake is additive (different slices) via host rescue `#rescueCompactionDeadEnd` (`session-maintenance.ts:2604`) or eager `chainShakeHint` (omp only). See [setup: Combining](setup.md#combining-omp-vcc-with-shake-and-snapcompact) and [harness §8.7](harness.md).
