...
`@zhulinchng/omp-vcc` — VCC-inspired algorithmic compaction plugin for `oh-my-pi` (`omp`). Zero-LLM, deterministic (30–470 ms), ports `sting8k/pi-vcc@0.7.0` into `oh-my-pi` extensions. Implements VCC views from `arxiv:2603.29678`: `V_full` identity, `V_ui` one-line brief transcript, `V_adapt(b,ρ)` ranked recall.
