...
describe("@zhulinchng/omp-vcc lifecycle", () => {
  it("registers without throwing", async () => {
    const { api, __handlers } = makeMockApi() as unknown as {
      api: never;
  ...
  it("probe header support routing (noop vs sync)", () => {
    const noopCtx = makeMockCtx({ headerMode: "noop" }) as unknown as {
      ui: { setHeader: (f: unknown) => void };
    };
    const syncCtx = makeMockCtx({ headerMode: "sync" }) as unknown as {
      ui: { setHeader: (f: unknown) => void };
    };
